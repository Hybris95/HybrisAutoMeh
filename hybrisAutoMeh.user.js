...
(function () {
  var init = function () {
    ...
    {
      window.log('HybrisAutoMeh waiting for loading-box...');
  ...
  loader = function () {
    if (!loaded && trys < (timeout * 1000 / retryspeed))
    {
      window.log('HybrisAutoMeh loading!');
      if (document.getElementsByClassName('loading-box').length > 0) {
        setTimeout(loader, retryspeed);
      } else {
        var script = document.createElement('script');
        script.textContent = '$.getScript(\'https://rawgit.com/Hybris95/HybrisAutoMeh/master/hybrisAutoMeh.js\');';
        document.head.appendChild(script);
        window.log('HybrisAutoMeh loaded!');
        loaded = true;
      }
    }
  };
